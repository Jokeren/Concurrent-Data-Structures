...
import com.jokeren.concurrent.structures.quadtree.KeySet;
...
public class ThreadLoopTime extends Thread implements ThreadMisc {
    ...
    final private ConcurrentHashTrie ctrie;
    ...
    public ThreadLoopTime(int threadId, Long[] keys, int insertRatio, int removeRatio, int containRatio,
                          int moveRatio, int nonUniform, int range, CyclicBarrier gate, ConcurrentHashTrie ctrie) {
    ...
    public void run() {
        ...
        ctrie.putIfAbsent(keySet, new Object());
        ctrie.remove(keySet);
        try {
            gate.await();
