...
public class ThreadLoopTime extends Thread implements ThreadMisc {
    ...
    public ThreadLoopTime(int threadId, KeySet[] keys, int insertRatio, int removeRatio, int containRatio,
    ...
    public void run() {
        ...
        if (operation < insertRatio) {
            ...
            KeySet moveKeySet = keys[ThreadLocalRandom.current().nextInt(range)];
            if (moveKeySet.getKeyX() != keySet.getKeyX() ||
                    moveKeySet.getKeyY() != keySet.getKeyY()) {
                quadtree.move(keySet.getKeyX(), keySet.getKeyY(),
                        moveKeySet.getKeyX(), moveKeySet.getKeyY());
                end = System.nanoTime();
                moveResponseTime += end - start;
